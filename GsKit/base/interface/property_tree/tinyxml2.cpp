...
XMLError XMLDocument::LoadFromMemory( const unsigned char *mem,
                                      const int numBytes )
{
    Clear();

    _charBuffer = new char[numBytes+1];
    memcpy((void*)_charBuffer, (void*)mem, numBytes);
    _charBuffer[numBytes] = 0;
