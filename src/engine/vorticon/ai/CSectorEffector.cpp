...
void CSectorEffector::process()
{
	switch(setype)
	{
	case SE_EXTEND_PLATFORM: se_extend_plat(PlatExtending ); break;
	case SE_RETRACT_PLATFORM: se_retract_plat(PlatExtending ); break;
	...
	default:
		g_pLogFile->ftextOut("Invalid sector effector type %d", type);
		break;
	}

}

void CSectorEffector::se_extend_plat(bool &PlatExtending)
{
#define PLAT_EXTEND_RATE        3
	...
	if (needinit)
	{
		...
		{
			type = SE_RETRACT_PLATFORM;
			se_retract_plat(PlatExtending);
			return;
		}
	...
	if (!timer)
	{
				...
				!TileProperties[mp_Map->at(platx, platy)].bleft)
		{
			...
			platx++;
			kill_all_intersecting_tile(platx, platy);
...
void CSectorEffector::se_retract_plat(bool &PlatExtending)
{
	if (needinit)
	{
		// figure out which direction the bridge is supposed to go
...
#define BG_GREY					143
	...
	if (touchPlayer && state == SPARK_ANIMATE)
	{
		...
		if (zapped)
		{
					...
					; obj != m_Objvect.end() ; obj++)
			{
				if ((*obj)->m_type==OBJ_SECTOREFFECTOR &&\
						(*obj)->type==SE_MORTIMER_SPARK &&\
						(*obj)->exists)
				{
					if ((*obj)->m_index!=m_index)
					{	// other sparks still exist
						type = SE_MORTIMER_RANDOMZAPS;
						needinit = true;
						return;
					}
				}
		...
		if (!timer)
		{
			...
			for(x=0;x<3;x++)
			{
				...
				if (mp_Map->at(mx, my) != 169)
				{
					...
					// spawn a ZAP! or a ZOT!
					CObject *newobject = new CObject(mp_Map);
					newobject->spawn(((mx<<4)+4)<<STC, my<<4<<STC, OBJ_RAY, m_Episode);
					newobject->ai.ray.state = RAY_STATE_SETZAPZOT;
...
void CSectorEffector::se_mortimer_heart()
{
		...
		if (zapped)
		{
			...
			for( ; it_obj!=m_Objvect.end() ; it_obj++ )
			{
				if((*it_obj)->m_type == OBJ_SECTOREFFECTOR &&
						(*it_obj)->type == SE_MORTIMER_HEART ) continue;
				else
				{
					deleteObj(**it_obj);
				}
	...
	case HEART_ZAPSRUNUP:
		...
		{	// spawn another wave of zaps
			...
			newobject->type = SE_MORTIMER_ZAPSUP;
	...
	case HEART_ZAPSRUNDOWN:
		if (!timer)
		{
			for(x=MORTIMER_MACHINE_XSTART;x<MORTIMER_MACHINE_XEND;x++)
			{
				...
				// spawn a ZAP! or a ZOT!
				CObject *newobject = new CObject(mp_Map);
				newobject->spawn(((x<<4)+4)<<STC, my<<4<<STC, OBJ_RAY, m_Episode);
				newobject->ai.ray.state = RAY_STATE_SETZAPZOT;
				newobject->ai.ray.direction = DOWN;
				newobject->inhibitfall = true;
				newobject->needinit = false;
