 ...
 *      Author: gerstrong
 *
 *  Updated on: 32.03.2010
 *  Now it is a base class for controls
 *  If you want to use a specific controls like the slider
 *  or create a new one, use that class as a base class
 *  for those.
 */
...
// This is a special add. It will be used in future implementations
// CDlgObject is a generic object for other controls...
void CDialog::addObject(CDlgObject *p_DlgObject, Uint16 x, Uint16 y)
{
	// If the stack is empty, set focus to the first element
	if(m_dlgobject.empty()) p_DlgObject->setFocus();

	// Put the ID, coordinates and width, to make it fit into the Dialogbox
	// then add it to the stack
	p_DlgObject->create( m_dlgobject.size(), m_x+(x*8), m_y+(y*8) );
	m_dlgobject.push_back(p_DlgObject);
}
...
{
	// TODO: We really should get rid of that implementation.
	// I will see what I can do...
	// they are processed by ID of course
	Uint8 max;
	if( (int) m_dlgobject.size() < m_h-2 )
	...
	{
		if(m_dlgobject[i]->m_selected)
			m_dlgobject[i]->processInput();

		// Check if we are about to select the next or prev item
		// The currently selected item decides that!
		if( m_dlgobject[i]->getPrevItem() )
		{
			int j=i;
			m_dlgobject[i]->looseFocus();
			do
			{
				if(j==m_scroll && j>0) m_scroll--;
				...
				{
					if( (int) m_dlgobject.size() < m_h-2 )
						m_scroll = m_dlgobject.size() - 1;
					else
						m_scroll = m_dlgobject.size() - m_h+2;

					j=m_dlgobject.size()-1;
				}
			}while(!m_dlgobject[j]->setFocus());

			m_selected_ID = j;
		}
		else if( m_dlgobject[i]->getNextItem() )
		{
			int j=i;
			m_dlgobject[i]->looseFocus();
			do
	...
	// Explanation of what the m_key variable represents...
	// 't' == text input
	// 'i' == integer input
	// 'n' == do nothing
	...
	else if(m_key != 'n')
	{
		...
		if(m_key == 's')
		{
			...
			{
				// TODO: Here was the slider. hopefully won't be needed anymore
			}
		}
	}*/
}

void CDialog::draw(SDL_Surface* sfc)
{
	// Fade-in effect
	if(m_alpha < 230)
	{
	...
	if(m_Frame) m_Frame->draw(sfc);
	
	CFont &Font = g_pGfxEngine->getFont();
	// Draw the up or down icon accordingly
	if( m_scroll>0 ) // Up Arrow
	{
		Font.drawCharacter(sfc, 15,
	...
	for(Uint16 i=m_scroll ;	i<max ; i++)
	{
		m_dlgobject[i]->render(sfc, m_scroll, 2*(i==m_selected_ID) );
	}
...
#define TWIRL_TIME	5
void CDialog::drawTwirl(SDL_Surface* sfc)
{
	...
	g_pGfxEngine->getFont().drawTwirl( sfc, m_twirl.frame,
