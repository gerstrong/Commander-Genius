...
void CObjectAI::nessie_ai(CObject& object)
{
	...
	if (object.needinit)
	{
		...
		object.ai.nessie.tiletrailX[0] = (object.x>>4>>CSF);
		object.ai.nessie.tiletrailY[0] = (object.y>>4>>CSF);
		object.ai.nessie.tiletrailX[1] = (object.x>>4>>CSF)+1;
		object.ai.nessie.tiletrailY[1] = (object.y>>4>>CSF);
...
void CObjectAI::nessie_find_next_checkpoint(CObject& object)
{
	...
	for(ya=0;ya<3;ya++)
	{
		for(xa=0;xa<3;xa++)
		{
			...
			obj = mp_Map->getObjectat(destx ,desty);
	...
	object.ai.nessie.destx = (destx<<4<<CSF);
	object.ai.nessie.desty = (((desty<<4)-8)<<CSF);
