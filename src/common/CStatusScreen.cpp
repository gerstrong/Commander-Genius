...
void CStatusScreen::drawInventoryEp1()
{
	...
	Font.drawFont( mp_surface, getstring("EP1_StatusBox"), (dlgX+1)<<3, (dlgY+1)<<3, true);
	...
	if (mp_inventory->HasCardYellow)
	{
		...
		if(mp_inventory->HasCardYellow > 1)
			Font.drawFont( mp_surface, itoa(mp_inventory->HasCardYellow),(dlgX+20)<<3,((dlgY+8)<<3)+3, false);
	}
	if (mp_inventory->HasCardRed)
	{
		...
		if(mp_inventory->HasCardRed > 1)
			Font.drawFont( mp_surface, itoa(mp_inventory->HasCardRed),(dlgX+24)<<3,((dlgY+8)<<3)+3, false);
	}
	if (mp_inventory->HasCardGreen)
	{
		...
		if (mp_inventory->HasCardGreen > 1)
			Font.drawFont( mp_surface, itoa(mp_inventory->HasCardGreen),(dlgX+20)<<3,((dlgY+10)<<3)+3, false);
	}
	if (mp_inventory->HasCardBlue)
	{
		...
		if(mp_inventory->HasCardBlue > 1)
			Font.drawFont( mp_surface, itoa(mp_inventory->HasCardBlue),(dlgX+24)<<3,((dlgY+10)<<3)+3, false);
	}
	// ship parts
	...
	if (i>999) i=999;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+4)<<3, (dlgY+12)<<3, false);
	
	// score
	i = mp_inventory->score;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+11-tempbuf.size())<<3, (dlgY+2)<<3, false);
	// extra life at
	i = mp_inventory->extralifeat;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+27-tempbuf.size())<<3, (dlgY+2)<<3, false);
	// lives
...
void CStatusScreen::drawInventoryEp2()
{
	...
	g_pGfxEngine->drawDialogBox( mp_surface, dlgX,dlgY,dlgW,dlgH, SDL_MapRGB(mp_surface->format, 172, 172, 172));
	Font.drawFont( mp_surface, tempbuf, (dlgX+1)<<3, (dlgY+1)<<3, true);
	...
	if (mp_inventory->HasCardRed)
	{
		...
		if(mp_inventory->HasCardRed > 1)
			  Font.drawFont( mp_surface, itoa(mp_inventory->HasCardRed),(dlgX+25)<<3,(dlgY+8)<<3);
	...
	if (mp_inventory->HasCardGreen)
	{
		...
		if(mp_inventory->HasCardGreen > 1)
			  Font.drawFont( mp_surface, itoa(mp_inventory->HasCardGreen),(dlgX+21)<<3,(dlgY+10)<<3);
	}
	if (mp_inventory->HasCardBlue)
	{
		...
		if(mp_inventory->HasCardBlue > 1)
			  Font.drawFont( mp_surface, itoa(mp_inventory->HasCardBlue),(dlgX+25)<<3,(dlgY+10)<<3);
	}
	...
	if (i>999) i=999;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+27-tempbuf.size())<<3, ((dlgY+5)<<3)-1);

	// score
	i = mp_inventory->score;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+11-tempbuf.size())<<3, (dlgY+2)<<3);
	// extra life at
	i = mp_inventory->extralifeat;
	tempbuf = itoa(i);
	Font.drawFont( mp_surface, tempbuf, (dlgX+27-tempbuf.size())<<3, (dlgY+2)<<3);
	// lives
	i = mp_inventory->lives;
	x = ((dlgX + 1)<<3)+4;
	if (i>7) i=7;
	...
	// cities saved
	if (mp_level_completed[4]) Font.drawFont( mp_surface, getstring("EP2_LVL4_TargetName"), (dlgX+1)<<3, (dlgY+8)<<3);
	if (mp_level_completed[6]) Font.drawFont( mp_surface, getstring("EP2_LVL6_TargetName"), (dlgX+8)<<3, (dlgY+8)<<3);
	if (mp_level_completed[7]) Font.drawFont( mp_surface, getstring("EP2_LVL7_TargetName"), (dlgX+1)<<3, (dlgY+9)<<3);
	if (mp_level_completed[13]) Font.drawFont( mp_surface, getstring("EP2_LVL13_TargetName"), (dlgX+8)<<3, (dlgY+9)<<3);
	if (mp_level_completed[11]) Font.drawFont( mp_surface, getstring("EP2_LVL11_TargetName"), (dlgX+1)<<3, (dlgY+10)<<3);
	if (mp_level_completed[9]) Font.drawFont( mp_surface, getstring("EP2_LVL9_TargetName"), (dlgX+8)<<3, (dlgY+10)<<3);
	if (mp_level_completed[15]) Font.drawFont( mp_surface, getstring("EP2_LVL15_TargetName"), (dlgX+1)<<3, (dlgY+11)<<3);
	if (mp_level_completed[16]) Font.drawFont( mp_surface, getstring("EP2_LVL16_TargetName"), (dlgX+8)<<3, (dlgY+11)<<3);
}
...
void CStatusScreen::drawInventoryEp3()
{
	...
	Font.drawFont( mp_surface, itoa(ankhtimepercent), (dlgX+7)<<3, ((dlgY+8)<<3)+7);
	...
	if (mp_inventory->HasCardYellow)
	{
		...
		if(mp_inventory->HasCardYellow > 1)
			Font.drawFont( mp_surface, itoa(mp_inventory->HasCardYellow),(dlgX+12)<<3,((dlgY+8)<<3)+3);
	}
	if (mp_inventory->HasCardRed)
	{
		...
		if(mp_inventory->HasCardRed > 1)
			Font.drawFont(mp_surface, itoa(mp_inventory->HasCardRed),(dlgX+16)<<3,((dlgY+8)<<3)+3);
	}
	if (mp_inventory->HasCardGreen)
	{
		...
		if(mp_inventory->HasCardGreen > 1)
			Font.drawFont(mp_surface, itoa(mp_inventory->HasCardGreen),(dlgX+20)<<3,((dlgY+8)<<3)+3);
	}
	if (mp_inventory->HasCardBlue)
	{
		...
		if(mp_inventory->HasCardBlue > 1)
			Font.drawFont(mp_surface, itoa(mp_inventory->HasCardBlue),(dlgX+24)<<3,((dlgY+8)<<3)+3);
	}
